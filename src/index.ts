...
import { getActivitiesLambda } from './lambdas/activities-get';
...
const sslCertificateValidationDnsRecord = new aws.route53.Record(
  'testapi-ssl-cert-validation-dns-record',
  {
...
const apiDomainName = new aws.apigateway.DomainName('testapi-api-domain-name', {
  certificateArn: validatedSslCertificate.certificateArn,
  domainName: domain,
});

const dnsRecord = new aws.route53.Record('testapi-api-dns', {
  zoneId: 'Z08201573ODBLNRIX8V6U',
  type: 'A',
  name: domain,
  aliases: [
...
const gw = new awsx.apigateway.API('testapi-test-api', {
  routes: [
    {
      path: '/activities',
      method: 'GET',
      eventHandler: getActivitiesLambda,
    },
  ],
});

const basePathMapping = new aws.apigateway.BasePathMapping('testapi-api-domain-mapping', {
  restApi: gw.restAPI,
